  ...
  if falar == "S":
    ...
    if decisao == "1":
      ...
      if ponte == "P":
        ...
        fino = input("\nEncontras outro caminho, porém é no rio e vais ter que saltar de pedra em pedra com peixes assassinos. (Dica: Tens comida na mochila) O que vais fazer? (Passar sem medo (PSM) | Dar a tua comida (DA)")
        if fino == "PSM":
          while fino:
          ...
          input("Boa Einstein! Deste de comer aos peixes e agora podes passar à vontade!")
    elif decisao == "2":
    ...
    print(mochila)
    print("\nQual opção escolhes? ( Esquerda (E) | Direita (D): ")
    opcao = input("OPÇÃO AQUI: ")
    if opcao == "E":
      porta1 = input("\nOh não! Deste de frente com um lobisomen! Para passar por ele vais ter de atacar ou fugir. Qual escolhes? ")
      if porta1 == "atacar" or porta1 == "Atacar":
...
if mescolha == "E":
  ...
  if croc == "F":
    ...
    print(mochila)
mescolha1 = input("\nEncontras-te mais 2 portas. Escolhe (Esquerda (E) | Direita (D)): ")
if mescolha == "E":
    input("\nEncontraste uma bruxa muito estranha que começou a falar uma língua estranha.")
    input(f"\n{nome}: Olá? Pode me ajudar a encontrar os meus amigos?.")
    input("\nBruxa: ...")
